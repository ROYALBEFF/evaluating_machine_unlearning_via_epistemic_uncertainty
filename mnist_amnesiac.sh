...
do
  ...
  do
    ...
    mlflow run . -e forgetting -P seed="$s" -P model="./mnist/models/${model}/pre-trained.pth" -P dataset=mnist -P target=3 -P percentage="$p" -P forgetting=amnesiac -P history="./mnist/batch_updates/${model}/batch_updates.pkl" -P output=./mnist/
  done
done
